...
const statusRadiosCriar = modalCriar.querySelectorAll('input[name="status"]');
...
const editStatusRadios = document.querySelectorAll('input[name="edit_status"]');
...
function criarCardAmbiente(ambiente) {
    const statusText = ambiente.status == 1 ? 'Ativo' : 'Inativo';
    const statusClass = ambiente.status == 1 ? 'ativo' : 'inativo';
    const statusIconClass = ambiente.status == 1 ? 'bi-check-circle-fill' : 'bi-x-circle';
    ...
    return `
        <div class="info_docente" data-id="${ambiente.id}">
            ...
            <div class="funcoes">
                ...
                <button class="status_docente ${statusClass}" data-id="${ambiente.id}" data-status="${ambiente.status}">
...
async function handleFormSubmit(event) {
    ...
    const statusAmbiente = document.querySelector('input[name="status"]:checked').value;
    const payload = {
        ...
        status: parseInt(statusAmbiente),
    };
...
async function carregarDadosParaEdicao(ambienteId) {
    try {
        ...
        editStatusRadios.forEach(radio => {
            if (parseInt(radio.value) === ambiente.status) {
                radio.checked = true;
            }
        });
